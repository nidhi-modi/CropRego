...
export default class Har1HTLPlant1 extends React.Component {
    ...
    componentDidMount() {
        ...
        this.focusListener = this.props.navigation.addListener('focus', () => {
            ...
            setTimeout(() => {
                ...
                db.trussByWeekNumberRowAndName(no, numberWeek, 'HAR 1 - HTL1606242', '125').then((data1) => {
                    console.log(data1);
                    console.log("Calling database")
    ...
    savePlantsToDbAlert = () => {
        ...
        if (this.state.isError) {
            ...
            Alert.alert(
                ...
                [
                    { text: 'No', onPress: () => this.props.navigation.navigate('Har1HTLDataChecker', { plantNo: no }), style: 'cancel' },
                                    ...
                                    if (stmDiameter) {
                                        if (lastWeekStmDiameter) {
                                            ...
                                            if (this.state.isItConnected === 'Online') {
                                                ...
                                                const url = `${scriptUrl}?
                                                callback=ctrlq&plantRow=${'125'}&plantName=${'HAR 1 - HTL1606242'}&plantWeek=${numberWeek}&plantNumber=${no}&leaves=${leavesPerPlant}&fullySetTruss=${fullySetTruss}&setTrussLength=${setTrussLength}&weeklyGrowth=${weeklyGrowth}&flowerHeight=${floweringTrussHeight}&leafLength=${leafLength}&leafWidth=${leafWidth}&stmDia=${stmDiameter}&lastWkStmDia=${lastWeekStmDiameter}`;
                                                ...
                                                db.addPlants(data1).then((result) => {
                                                    ...
                                                    this.props.navigation.navigate('Har1HTL')
                                                    this.setState({

                                                        isDataSend: true,
                                                    });
                                                    abc = '1';
                                                    this.setItem('Har1HTLPlant', 1)
                    ...
                    <ImageBackground source={require('../assets/background2.png')} style={styles.backgroundImage}>
                        ...
                        <ScrollView style={styles.formContainer}
                            ...
                            <Text style={styles.text} onChangeText={(text) => this.updateTextInput(text, 'plantRow')}
                                value={this.state.plantRow}>Row No 125 / Week No {numberWeek}</Text>
                            ...
                            {this.state.checkWeeksTrussData !== 1 ? (
                                <TouchableOpacity
                                    style={styles.buttonContainer}
                                    disabled={false}
                                    onPress={() => this.props.navigation.navigate('Har1HTLTrussDetails', { plantNum: no })}>
                            ...
                            <TouchableOpacity onPress={() => this.props.navigation.navigate('Har1HTLDataChecker', { plantNo: no })}>
