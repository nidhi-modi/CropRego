...
import { StyleSheet, View, Text, TouchableOpacity, ActivityIndicator, TextInput, ImageBackground, Image, SafeAreaView, Alert, ToastAndroid, AlertIOS, Platform } from 'react-native'
...
export default class Har1AngellePlant1 extends React.Component {
    ...
    componentDidMount() {
        ...
        this.focusListener = this.props.navigation.addListener('focus', () => {
            ...
            setTimeout(() => {
                ...
                db.trussByWeekNumberRowAndName(no, numberWeek, 'HAR 1 - Angelle', '155').then((data1) => {
                    console.log(data1);
                    console.log("Calling database")
        ...
        if (this.state.isError) {
            ...
            Alert.alert(
                ...
                [
                    ...
                    { text: 'No', onPress: () => this.props.navigation.navigate('Har1AngelleDataChecker', { plantNo: no }), style: 'cancel' },
                                    ...
                                    if (stmDiameter) {
                                        if (lastWeekStmDiameter) {
                                            ...
                                            if (this.state.isItConnected === 'Online') {
                                                ...
                                                const url = `${scriptUrl}?
                                                callback=ctrlq&plantRow=${'155'}&plantName=${'HAR 1 - Angelle'}&plantWeek=${numberWeek}&plantNumber=${no}&leaves=${leavesPerPlant}&fullySetTruss=${fullySetTruss}&setTrussLength=${setTrussLength}&weeklyGrowth=${weeklyGrowth}&flowerHeight=${floweringTrussHeight}&leafLength=${leafLength}&leafWidth=${leafWidth}&stmDia=${stmDiameter}&lastWkStmDia=${lastWeekStmDiameter}`;
                    ...
                    <ImageBackground source={require('../assets/background2.png')} style={styles.backgroundImage}>
                        ...
                        <ScrollView style={styles.formContainer}
                            ...
                            <Text style={styles.text} onChangeText={(text) => this.updateTextInput(text, 'plantRow')}
                                value={this.state.plantRow}>Row No 155 / Week No {numberWeek}</Text>
                            ...
                            {this.state.checkWeeksTrussData !== 1 ? (
                                <TouchableOpacity
                                    style={styles.buttonContainer}
                                    disabled={false}
                                    onPress={() => this.props.navigation.navigate('Har1AngelleTrussDetails', { plantNum: no })}>
                                ...
                                <TouchableOpacity onPress={() => this.props.navigation.navigate('Har1AngelleDataChecker', { plantNo: no })}>
